...
def test_run_stdio_uses_resolved_command_and_prepended_path(tmp_path):
    ...
    async def _test():
        with patch("tools.mcp_tool.shutil.which", return_value=None), \
            ...
            # The real (resolved) command on POSIX is wrapped in the
            # parent-death watchdog supervisor. On Windows the watchdog is a
            # no-op (process groups), so the resolved npx path is passed
            # through directly (optionally rewritten off .cmd shims).
