...
def run_conversation(
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # Derive the char count from the cheap token estimate instead of
        # re-stringifying the entire message history every API call:
        # sum(len(str(msg)) ...) materialized hundreds of KB of dict reprs per
        # iteration on 150-250K-token histories, a tight GIL-holding loop that
        # starved the uvicorn event-loop thread (>10s "loop stalled" warnings,
        # session.create timeouts). estimate_request_tokens_rough still adds the
        # tool schema payload back so compression/pressure decisions see the
        # full request, not just the message history.
