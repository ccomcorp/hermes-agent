...
def inject_turn_context(api_msg, ext_prefetch_cache, plugin_user_context, agent) -> bool:
    """Compatibility seam for current-turn memory/plugin context injection.

    The turn prologue now stamps this with ``compose_user_api_content`` in
    ``agent.turn_context``, but AIOS seam tests and local integrations still
    exercise this named helper directly. Keep it as a thin wrapper around the
    canonical composer, and confirm prefetch consumption only when the memory
    block actually reached a string API prompt.
    """
    composed = compose_user_api_content(api_msg.get("content", ""), ext_prefetch_cache, plugin_user_context)
    if composed is None:
        return False

    api_msg["content"] = composed
    mem_injected = bool(ext_prefetch_cache and build_memory_context_block(ext_prefetch_cache))
    if mem_injected:
        mm = getattr(agent, "_memory_manager", None)
        if mm is not None:
            try:
                # AIOS-LOOP-SEAM:injection-confirm — confirm the prefetched memory
                # block was injected into this turn's API prompt (loop consume leg).
                    ...
                    if _ext_prefetch_cache:
                        ...
                        if _mm is not None:
                            try:
                                # AIOS-LOOP-SEAM:injection-confirm — confirm the prefetched memory
                                # block was injected into this turn's API prompt (loop consume leg).
                                _mm.confirm_prefetch_consumed(getattr(agent, "session_id", "") or "")
                            except Exception:
                                pass
                else:
