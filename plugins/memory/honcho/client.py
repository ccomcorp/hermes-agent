...
def resolve_global_config_path() -> Path:
    ...
    On Windows, ``Path.home()`` and ``expanduser('~')`` prefer USERPROFILE and
    ignore a test/subprocess-scoped HOME. Honor HOME explicitly first so the
    Honcho config path follows the same scoped home the caller set up.
    """
    home = os.environ.get("HOME", "").strip()
    base = Path(home).expanduser() if home else Path.home()
    return base / ".honcho" / "config.json"
